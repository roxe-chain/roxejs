...
import { Api } from '../roxejs-api';
import { JsonRpc } from '../roxejs-jsonrpc';
import { JsSignatureProvider } from '../roxejs-jssig';
...
describe('roxejs-api', () => {
    ...
    beforeEach(() => {
        ...
        const signatureProvider = new JsSignatureProvider(['5KS2QMfShmDjHaLAZEPJgehVXAobgo5YfVw1mzPBHaPpGfKbkZL']);
    ...
    it('getAbi returns an abi', async () => {
        const response = await api.getAbi('testroxetoken');
    ...
    it('getContract returns a contract', async () => {
        const response = await api.getContract('testroxetoken');
